...
def print_payment_usage() -> None:
    ...
    print("  PAYMENT_CSV          Path to payment CSV (default: wisr_payment_test.csv)")
...
def _resolve_csv_path(csv_file: str, script_dir: Path) -> str:
    """Resolve relative csv_file against script_dir or cwd."""
...
def parse_payment_cli_args(script_dir: Path) -> str:
    ...
    if not args or args[0] in ("-h", "--help"):
        ...
        return _resolve_csv_path(csv_file, script_dir)
    csv_file = args[0]
    return _resolve_csv_path(csv_file, script_dir)
