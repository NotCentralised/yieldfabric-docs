...
# Test script for the List Annuities API endpoint
# Demonstrates how to call GET /api/annuities to retrieve annuities linked to the caller
...
list_annuities() {
    ...
    echo_with_color $BLUE "  🌐 Making REST API request to: ${PAY_SERVICE_URL}/api/annuities" >&2

    local temp_file=$(mktemp)
    local http_code=$(curl -s -w "%{http_code}" -o "$temp_file" \
        -H "Authorization: Bearer ${jwt_token}" \
        "${PAY_SERVICE_URL}/api/annuities")
...
main() {
    echo_with_color $CYAN "🚀 Starting List Annuities API Test"
    echo ""
    ...
    echo_with_color $CYAN "📤 Step 2: Calling list annuities endpoint..."
    echo ""
    local response=$(list_annuities "$jwt_token")
    ...
    if [[ "$status" == "success" ]]; then
        echo_with_color $GREEN "✅ Annuities retrieved successfully!"
        echo ""
        echo_with_color $PURPLE "📦 Annuities:" 
        echo "$result" | jq '.' | sed 's/^/  /'
        echo ""
        echo_with_color $GREEN "🎉 List Annuities API Test Completed Successfully!"
        return 0
    else
        echo_with_color $RED "❌ Failed to retrieve annuities!"
        echo ""
        echo_with_color $YELLOW "Error: $error"
        echo ""
        echo_with_color $RED "❌ List Annuities API Test Failed"
        return 1
    fi
}
