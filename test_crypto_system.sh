...
echo -e "\n5️⃣ Creating Test Group for Crypto Operations..."
...
echo -e "\n7️⃣ Testing Crypto Flow - Local Encryption..."
...
echo -e "\n8️⃣ Testing Crypto Flow - Remote Decryption..."
...
if [ $? -eq 0 ] && echo "$DECRYPT_RESPONSE" | jq -e '.result' >/dev/null 2>&1; then
    ...
    echo "   ⚠️  Remote decryption failed (may require Phase 2 implementation)"
...
echo -e "\n9️⃣ Testing Crypto Flow - Remote Signing..."
...
if [ -n "$SIGNATURE" ] && [ "$SIGNATURE" != "null" ]; then
    echo "   ✍️  Signature: ${SIGNATURE:0:30}..."
    echo "   🔑 Key ID: $USER_KEY_ID"
    echo "   🔧 Provider Type: OpenSSL"
    ...
    echo "   ⚠️  Skipping signature verification (no signature available)"
...
echo -e "\n1️⃣1️⃣ Testing Group Key Operations with Delegation JWT..."
...
if [[ "$DELEGATION_GROUP_ID" != "$GROUP_ID" ]]; then
    echo "   ⚠️  Delegation JWT is for a different group, creating new delegation JWT for current group..."
...
echo "   ✍️  Testing group key signing with delegation JWT..."
...
echo -e "\n1️⃣2️⃣ Testing Security Restrictions - No Delegation = No Access..."
...
echo -e "\n1️⃣3️⃣ Testing Real Signature Authentication..."
...
if [ "$HTTP_STATUS" = "200" ]; then
    if [ -n "$RESPONSE_BODY" ] && echo "$RESPONSE_BODY" | jq -e '.result' >/dev/null 2>&1; then
        ...
        echo "   ✍️  Signature: ${VAULT_SIGNATURE:0:50}..."
        ...
        if [ "$DECRYPT_HTTP_STATUS" = "200" ]; then
            ...
            echo "   ⚠️  Vault decrypt endpoint test completed"
            echo "   📊 HTTP Status: $DECRYPT_HTTP_STATUS"
            echo "   📄 Response: $DECRYPT_RESPONSE_BODY"
        fi
        
    elif [ -n "$RESPONSE_BODY" ]; then
        echo "   ⚠️  Vault signature creation returned unexpected response format"
        echo "   📄 Response: $RESPONSE_BODY"
        echo "   💡 The endpoint is working but response format is different than expected"
    else
        echo "   ⚠️  Vault signature creation returned empty response body"
...
if [ $? -eq 0 ] && echo "$INVALID_VERIFY_RESPONSE" | jq -e '.success' >/dev/null 2>&1; then
    ...
    if [ "$SUCCESS_RESULT" = "false" ]; then
        ...
        echo "   ⚠️  Invalid signature verification accepted (unexpected)"
        echo "   📄 Response: $INVALID_VERIFY_RESPONSE"
...
if [ "$HTTP_STATUS" = "200" ]; then
    if [ -n "$RESPONSE_BODY" ] && echo "$RESPONSE_BODY" | jq -e '.result' >/dev/null 2>&1; then
        ...
        echo "   ✍️  Signature: ${GROUP_VAULT_SIGNATURE:0:50}..."
        echo "   📊 This demonstrates group key operations through vault endpoint work with delegation"
    elif [ -n "$RESPONSE_BODY" ]; then
        echo "   ⚠️  Group vault signing returned unexpected response format"
        echo "   📄 Response: $RESPONSE_BODY"
        echo "   💡 The endpoint is working but response format is different than expected"
    else
        echo "   ⚠️  Group vault signing returned empty response body"
        echo "   💡 The endpoint is working (HTTP 200) but response body is empty"
...
echo -e "\n1️⃣4️⃣ Testing Public Key Retrieval..."
...
echo -e "\n1️⃣5️⃣ Testing Delegation JWT CryptoOperations Scope..."
...
echo -e "\n1️⃣6️⃣ Testing Integration Between Crypto Components..."
...
if [[ -n "$GROUP_ENCRYPTED_DATA" ]]; then
    ...
    if [ $? -eq 0 ] && echo "$GROUP_DECRYPT_RESPONSE" | jq -e '.result' >/dev/null 2>&1; then
        ...
        if [[ "$GROUP_DECRYPTED_DATA" == "Integration test message" ]]; then
            ...
            echo "   ⚠️  Integration test failed - data mismatch"
        fi
    else
...
echo -e "\n1️⃣7️⃣ Testing Advanced Permission Scenarios..."
...
if [ "$HTTP_STATUS" = "403" ] || [ "$HTTP_STATUS" = "401" ]; then
    ...
    echo "   ⚠️  Permission boundary may not be enforced"
    echo "   📊 HTTP Status: $HTTP_STATUS"
    echo "   📄 Response: $RESPONSE_BODY"
fi
    ...
    if [ -n "$CROSS_USER_ID" ] && [ "$CROSS_USER_ID" != "null" ]; then
        ...
        if [ -n "$GRANT_RESPONSE" ]; then
            ...
            if [ "$HTTP_STATUS" = "403" ] || [ "$HTTP_STATUS" = "401" ]; then
                ...
                echo "   ⚠️  Cross-user access control may not be working"
            fi
        else
...
echo "   🛡️  Test 17.3: Security Restriction Validation..."
...
if [ "$HTTP_STATUS" = "403" ] || [ "$HTTP_STATUS" = "401" ]; then
    ...
    echo "   ⚠️  Security restriction may not be enforced"
    echo "   📊 HTTP Status: $HTTP_STATUS"
    echo "   📄 Response: $RESPONSE_BODY"
fi
...
if [ "$HTTP_STATUS" = "403" ] || [ "$HTTP_STATUS" = "401" ]; then
    ...
    echo "   ⚠️  Delegation scope may not be properly enforced"
    echo "   📊 HTTP Status: $HTTP_STATUS"
    echo "   📄 Response: $RESPONSE_BODY"
fi
...
echo -e "\n1️⃣8️⃣ Cleaning Up - Deleting Test Group..."
...
echo -e "\n1️⃣9️⃣ Final Token Status Check..."
...
echo -e "\n🏗️  Crypto System Features Demonstrated:"
